...
const (
	maxOrificeAttemptsLeveling = 10
	orificeCheckDelayLeveling  = 200
)

var talTombsLeveling = []area.ID{area.TalRashasTomb1, area.TalRashasTomb2, area.TalRashasTomb3, area.TalRashasTomb4, area.TalRashasTomb5, area.TalRashasTomb6, area.TalRashasTomb7}

type DurielLeveling struct {
	ctx *context.Status
}

func NewDurielLeveling() *DurielLeveling {
	return &DurielLeveling{
		ctx: context.Get(),
	}
}

func (d DurielLeveling) Name() string {
	return string(config.DurielRun)
}

func (d DurielLeveling) Run() error {
...
func (d DurielLeveling) findRealTomb() (area.ID, error) {
	var realTomb area.ID

	for _, tomb := range talTombsLeveling {
